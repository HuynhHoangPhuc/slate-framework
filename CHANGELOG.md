...
### Fixed — cross-adapter device-removed crash (hybrid-GPU monitor migration)
- The per-redraw adapter-LUID probe and `on_display_change` now mark the device lost **unconditionally** on a confirmed `current_monitor_luid != current_adapter_luid` mismatch (via the shared `AppState::probe_adapter_mismatch_and_escalate`), instead of routing through the health-gated `mark_device_potentially_lost`. Recovery then rebuilds the renderer on the window's current monitor's adapter.
- Root cause: on a hybrid-GPU multi-monitor laptop the window can land on a monitor served by a different GPU than the renderer's startup adapter. The probe detected the mismatch but escalated via `mark_device_potentially_lost`, which consults `GetDeviceRemovedReason()` — a *healthy* device on the *wrong* adapter returns `S_OK`, so it never marked the device lost. The renderer kept cross-adapter DirectComposition-composing until DXGI hard-removed it (`0x887A0005`) with no graceful recovery — a random, no-interaction "crash" (verified from a real crash log: 25 mismatch events over 5s, `device_lost` never set, recovery state never left `NotLost`).
- The escalation now classifies as `LuidMigration` (the wgpu callback never fired), which bypasses the flap guard — correct for a user-driven monitor move. The migration is proactive: it fires while the device is still healthy, so recovery rebuilds on the right GPU *before* DXGI gives up, rather than after a hard removal. Verified on the repro laptop: mismatch → deferred during drag → `GPU device recovered successfully` (attempt 1/5) → no recurrence, no crash.
- Locked by `adapter_luid_mismatch_escalates_recovery` (Windows + test-hooks): an injected LUID mismatch with a healthy device must mark the device lost and engage recovery. Known residual (documented): if adapter selection cannot honor the window's monitor, the mismatch persists as a spaced rebuild loop rather than a crash — a repeated-migration circuit-breaker is the follow-up if it surfaces.
