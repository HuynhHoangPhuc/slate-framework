...
//!
//! Several test binaries in this crate require `test-hooks` and are otherwise
...
fn test_hooks_feature_must_be_enabled() {
    ...
    if !cfg!(feature = "test-hooks") {
        panic!(
            "the test-hooks feature is off, so the gated dispatch suites were skipped; \
             re-run with: cargo test -p slate-framework --features test-hooks"
        );
    }
}
